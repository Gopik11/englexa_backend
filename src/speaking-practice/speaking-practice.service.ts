...
import { ALL_SPEAKING_TOPICS, SPEAKING_TOPICS_BY_LEVEL } from '../content/speaking-topics.constants';
import {
...
import { SpeakingAudioReference, SpeakingEvaluator } from './utils/speaking-evaluator';
...
export class SpeakingPracticeService {
  ...
  async submitSpeakingAudio(
    ...
    if (!audio.audioUrl && !audio.audioBlobRef) {
      throw new BadRequestException(
        'Either audioUrl or audioBlobRef is required.',
      );
    }
    ...
    const userProgress = getOrCreateSpeakingTopicProgress(userId, topic, level);
    const result = await this.speakingEvaluator.evaluateSpeaking({
      userId,
      prompt,
      audio,
