...
export interface SpeakingAudioPayload {
  audioBase64: string;
  mimeType?: string;
}
...
export class SpeakingEvaluator {
  private readonly logger = new Logger(SpeakingEvaluator.name);
  ...
  async evaluateSpeaking(
    ...
    const referenceText = input.prompt.reference_text ?? input.prompt.prompt;
    ...
    const fluencyScore = this.assessFluency(
      transcript,
      referenceText,
    );

    const grammarFeedback = this.buildGrammarFeedback(transcript, referenceText);
    ...
    return {
      ...
      vocabularyFeedback,
      microLesson,
      aiDegraded: transcript === referenceText && !input.audio.audioBase64,
      ...enriched,
    };
  }
  ...
  private async transcribeAudio(
    ...
    if (audio.audioBase64?.trim()) {
      try {
        const stt = await this.aiProvider.speechToText(audio.audioBase64, {
          mimeType: audio.mimeType ?? 'audio/mp4',
        });
        if (stt.text.trim()) {
          this.logger.log(`STT transcript length=${stt.text.length}`);
          return stt.text.trim();
        }
      } catch (err) {
        this.logger.warn(
          `Whisper STT failed, using heuristic fallback`,
          err instanceof Error ? err.message : err,
        );
      }
    }

    return this.transcribeWithHeuristic(referenceText, audio.audioBase64);
  }

  private transcribeWithHeuristic(
    referenceText: string,
    audioKey: string,
  ): string {
  ...
  private assessFluency(transcript: string, referenceText: string): number {
    ...
    const raw = coverage * 55 + lengthRatio * 35 - fillerCount * 8;
  ...
  private buildGrammarFeedback(
    ...
    if (
      ...
      issues.push(
        'Include a clear verb form (for example, "is" or "are") so the sentence is grammatical.',
      );
    }
    if (
  ...
  private buildVocabularyFeedback(
    ...
    const stronger =
      prompt.key_vocabulary.find((item) => item !== target) ?? target;
